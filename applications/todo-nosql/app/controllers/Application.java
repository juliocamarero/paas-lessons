...
public class Application extends Controller {
  ...
  */
  private static DB getDb() {
      ...
      String remotePort = play.Configuration.root().getString("mongo.remote.port");

      String dbName = play.Configuration.root().getString("mongo.db");
      ...
      if(local){
        ...
        db = m.getDB( dbName );
          ...
          String hostname = remoteHostName;
            int port = Integer.valueOf(remotePort);
            try {
            m = new Mongo( hostname , port);   
            db = m.getDB( dbName );
